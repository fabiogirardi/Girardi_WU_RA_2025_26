...
estimate_model <- function(data, data.pc, dep.vars, add.vars, pca.cat.dict, model, type, time.col, data.freq, split.ratio, lags, pca, rolling.pca=TRUE, where.funcs, optional.args=FALSE, sparse.pca, alpha.pca, beta.pca, t1 = t1, t2 = t2, n_pc = n_pc, extra = NA){
  ...
  for (j in jj:jjj) {
    ...
    aaa = df.pc %>% filter(!!as.symbol(time.col) <= test.date)
    aaa <- cbind(aaa, projected) %>% filter(month == 12)
    ...
    train.test.data <- create_train_test_data(aaa, test.date, train.date, time.col, system.vars,
                                              as.time.series=TRUE, data.freq=data.freq, only.train=F, only.test=T)
