...
estimate_model_in_sample <- function(data, data.pc, dep.vars, add.vars, pca.cat.dict, time.col, data.freq, lags, pca, where.funcs, model, 
  ...
  if (pca){
    ...
    if (!is.na(extra)) {
      add.vars = c(add.vars,paste0("full_",extra))
    }
  ...
  for (j in jj:(length(add.vars)+1)) {
    ...
    if (j > 1 & j < 12) {
        ...
        if (pca){
          ...
          cat <- paste0(add.vars[j-1],"_pca_",seq(1:n_pc))
    ...
    if (j == 12) {
      ...
      if (pca){
        ...
        cat <- c(paste0(add.vars[j-2],"_pca_",seq(1:n_pc)), paste0(extra,"_pca_",seq(1:n_pc)))
    ...
    if (j > 1 && pca){
      ...
      if (sparse.pca==F  |  j < 11 ) {
        ...
        colnames(projected) <- paste0(add.vars[j-1],"_pca_", seq(1:n_pc))
      }
      
      if (sparse.pca==T  &  j == 11 ) {
        
        pca.fit <- spca(data.train.temp[pca.cat.dict[[add.vars[j-1]]]], center=TRUE, scale = TRUE, alpha = alpha.pca, beta = beta.pca, max_iter = 1000, verbose = F)
        pca.X =  scale(data.pc[pca.cat.dict[[add.vars[j-1]]]], center=T, scale = F)
        projected <- (as.matrix(pca.X) %*% pca.fit$loadings)[,1:n_pc]   %>% as.data.frame 
       
        colnames(projected) = paste0("PC",seq(1:n_pc))
        
        loadings_spca_full = pca.fit$loadings; 
        
        pca_temp <- prcomp(data.train.temp[pca.cat.dict[[add.vars[j-1]]]], center=TRUE, scale = TRUE)
        
        loadings_pca_full = pca_temp$rotation
        colnames(projected) <- paste0(add.vars[j-1],"_pca_", seq(1:n_pc))
      }
      
      if (sparse.pca==T  &  j == 12 ) {
        
        pca.fit <- spca(data.train.temp[pca.cat.dict[[add.vars[j-2]]]], center=TRUE, scale = TRUE, alpha = alpha.pca, beta = beta.pca, max_iter = 1000, verbose = F)
        pca.X =  scale(data.pc[pca.cat.dict[[add.vars[j-2]]]], center=T, scale = F)
        projected <- (as.matrix(pca.X) %*% pca.fit$loadings)[,1:n_pc]   %>% as.data.frame 
        colnames(projected) = paste0("PC",seq(1:n_pc),"_full")
        
        
        loadings_spca_full = pca.fit$loadings; 
        pca_temp <- prcomp(data.train.temp[pca.cat.dict[[add.vars[j-2]]]], center=TRUE, scale = TRUE)
        loadings_pca_full = pca_temp$rotation
        
        
       
        pca.fit <- prcomp(data.train.temp[pca.cat.dict[[extra]]], center=TRUE, scale = TRUE)
        pca.X =  scale(data.pc[pca.cat.dict[[extra]]], center=T, scale = F)
        projected1 <-  (as.matrix(pca.X) %*% pca.fit$rotation)[,1:n_pc]   %>% as.data.frame    
        colnames(projected1) = paste0("PC",seq(1:n_pc),"_",extra)
