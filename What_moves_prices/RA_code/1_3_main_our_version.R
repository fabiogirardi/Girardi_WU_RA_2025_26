...
oos = T
annualized = FALSE
path_functions = paste0("C:\\Users\\mholzman\\OneDrive - WU Wien\\Dokumente\\GitHub\\Girardi_WU_RA_2025_26\\What_moves_prices\\code\\version_2024\\")
lags = 1
fit_intercept = FALSE
...
load("portfolio_monthly.RData")


## ======================================= ##
## Choose factor portfolio (H, M, L, B, S) ##
ff_factor <- "L"

## filter the data frame for the chosen ff_factor
## port = column name indicating which of the 5 factors it is
df <- portfolio_monthly %>%
  filter(port == ff_factor, 
         year(quarter_id) >= 1980)


## Add equity premium variables from the Shiller dataset

data_equity <- read_csv("C:\\Users\\mholzman\\OneDrive - WU Wien\\Dokumente\\GitHub\\Girardi_WU_RA_2025_26\\What_moves_prices\\data\\earnings_dividends_price_ratios_monthly_public.csv") %>% as.data.frame()

df <- df %>%
  left_join(
    data_equity %>% dplyr::select(month_id, ntis, tbl, tms, svar, dfy, siioutofsample),
    by = "month_id"
  )
...
## we source a modified version of the data_preparation function to suit our data
source(file="data_preparation2.R", chdir = T)  ## create p/d, p/e, ed, and ret and they lead values

df.all.freq <- data_preparation(df, annualized = T) %>% filter(month_id >= startdate, month_id <= enddate ) %>% as.data.frame()
...
# this block is doing two main things:

# Computing time-varying (rolling) correlations between valuation ratios and future fundamentals/returns.

# Computing unconditional (full-sample) correlations for comparison.
...
#Correlation between price–dividend ratio and next-year dividend growth
...
#Correlation between p/d and next-year returns
...
#Correlation between price–earnings ratio and next-year dividend growth
...
#Correlation between p/e and next-year returns
...
#Correlation between p/d and 5-year-ahead dividend growth
...
#Correlation between p/d and 5-year-ahead returns
...
#Correlation between p/e and 5-year-ahead earnings growth
...
#Correlation between p/e and 5-year-ahead returns
...
fund_plt_1 = ggplot(data=df.all.freq[60:504,]) + theme_bw() +
  ...
  labs(color = '') + 
  xlab("Time") + ylab("Cor( pd , . )") + ylim(c(-1,1)) +
  geom_hline(yintercept = cor(df.all.freq$d_gr_5y_lead, df.all.freq$p_d, use = "complete.obs"), col = "blue", size = 1.2, linetype = "twodash") +
  geom_hline(yintercept = cor(df.all.freq$ret_5y_lead, df.all.freq$p_d, use = "complete.obs"), col = "darkred", size = 1.2, linetype = "twodash") +
  theme(legend.position = c(0.15, 0.20), legend.text = element_text(size=13))
...
fund_plt_2 = ggplot(data=df.all.freq[60:504,]) + theme_bw() +
  ...
  labs(color = '') + 
  xlab("Time") + ylab("Cor( pe , . )") + ylim(c(-1,1)) +
  geom_hline(yintercept = cor(df.all.freq$e_gr_5y_lead, df.all.freq$p_e, use = "complete.obs"), col = "chartreuse4", size = 1.2, linetype = "twodash") +
  geom_hline(yintercept = cor(df.all.freq$ret_5y_lead, df.all.freq$p_e, use = "complete.obs"), col = "darkred", size = 1.2, linetype = "twodash") +
  theme(legend.position = c(0.15, 0.20), legend.text = element_text(size=13))
...
# this block is about computing and visualizing how much variance each group of fundamental characteristics explains through their first few principal components (PCs)
...
spc_full_plot =  ggplot(data=df.freq) + theme_bw() +
  ...
  xlab("time") + ylab("SPc (full set)") +  ylim(c(-128,112)) +
...
pc_full_plot =  ggplot(data=df.freq) + theme_bw() +
  ...
  xlab("time") + ylab("PCs (full set)") + ylim(c(-130,115)) +
  theme(legend.position = c(0.10,0.20)) 
...
#http://127.0.0.1:32220/graphics/plot_zoom_png?width=1172&height=348
######################################################################################################
...
fund_plt_1 = ggplot(data = df.all.freq.pcs) + theme_bw() +
  ...
  xlab("time") + ylab("dividend growth") + ylim(c(-0.3,0.5)) +
...
source(file=paste0("C:\\Users\\kalaei\\OneDrive - WU Wien\\Dokumente\\What moves prices\\code\\version_2024\\estimate_model_in_sample_v5.R"), chdir = T)
source(file=paste0("C:\\Users\\kalaei\\OneDrive - WU Wien\\Dokumente\\What moves prices\\code\\version_2024\\estimate_model_in_sample_v6_always_full.R"), chdir = T)
...
for (i in 1:grid_sparse_pc_len) {
  ...
  var_insample = estimate_model_in_sample(data = df.freq, data.pc= df.freq, dep.vars = DEPVAR, add.vars = CHARS, pca.cat.dict = CATEGORIES, 
                                          ...
                                          where.funcs = "C:\\Users\\kalaei\\OneDrive - WU Wien\\Dokumente\\What moves prices\\code\\version_2024\\functions_estimation\\", 
