...
#
# Paper finds lambda_hat = 41 with SE 6 (in rate^-1 units).
...
# Use weekly data — already weekly via the OOS panel
...
cat("\nComputing block-bootstrap SE (300 reps, block size 13)...\n")
set.seed(42)
se_q <- block_boot_se(appC_data$rn_variance,
                      appC_data$rv_future,
                      appC_data$rn_third_moment,
                      n_boot = 300, block = 13)

cat(sprintf("SE (ppt^-1):       %.4f\n", se_q))
cat(sprintf("SE (rate^-1):      %.2f    [paper: 6]\n", se_q * 100))
cat(sprintf("t-statistic:       %.2f    [paper: > 5]\n", cl_q$lambda / se_q))
...
if (!is.null(acm_panel) && exists("acm_data")) {
